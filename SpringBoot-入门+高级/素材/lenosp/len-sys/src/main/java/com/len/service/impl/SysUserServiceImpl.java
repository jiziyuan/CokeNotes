...
public class SysUserServiceImpl extends BaseServiceImpl<SysUser, String> implements SysUserService {
    ...
    public int insertSelective(SysUser record) {
        ...
        // 保存用户
        int s = super.insertSelective(record);

        // 关联租户信息
        Map map = Maps.newHashMap();
        map.put("id", IDUtil.genId());
        map.put("tenant_id",Principal.getSession().getAttribute("globalTenantId"));
        map.put("user_id",record.getId());
        map.put("is_admin",0);
        map.put("create_date", DateUtil.date());
        // 保存租户信息
        sysUserMapper.insertTenant(map);
    ...
    public JsonUtil delById(String id, boolean flag) {
        ...
        try {
            ...
            // 删除租户中用户
            sysUserMapper.deleteTenantUser(sysUser.getId(),Principal.getTenantId());
    ...
    public List<Checkbox> getUserRoleByJson(String id) {
        ...
        sysRoleEntity.setTenantId((String)Principal.getSession().getAttribute("globalTenantId"));
        List<SysRole> roleList = roleService.selectListByPage(sysRoleEntity);
    ...
    public void setMenuAndRoles(String username) {
        ...
        // 查询租户
        List<Map> tenants = sysUserMapper.getUserTenants(currentUser.getId());
        System.err.println(JSON.toJSONString(tenants));
    ...
    @Override
    public List<Project> getProjects(String tenantId) {
        return sysUserMapper.getProjects(tenantId);
    }
}
