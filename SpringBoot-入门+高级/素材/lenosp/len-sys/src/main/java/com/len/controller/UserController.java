...
public class UserController extends BaseController {
    ...
    @ResponseBody
    @RequiresPermissions("user:show")
    public ReType showUser(Model model, SysUser user, int page, int limit) {
        user.setTenantId((String)Principal.getSession().getAttribute("globalTenantId"));
        long total = 0;
        List<SysUser> userList = Lists.newArrayList();
        if(page > 0){
            Page<?> tPage = PageHelper.startPage(page, limit);
            userList = userService.selectListByPage(user);
            total = tPage.getTotal();
        }else{
            userList = userService.selectListByPage(user);
            total = userList.size();
        }
        return new ReType(total,userList);
    }
